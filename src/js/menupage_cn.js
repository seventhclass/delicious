	...
	function createMenuPage(res){
		...
		if(res.dishinfo && res.dishinfo.length>0){
			$.each(res.dishinfo,function(i, item){
				$('#dishbox').append(
						"<li class='pic'>"
						+ "<img src='./image_350/" + item.album_path + "'" + " alt='" + item.dish_name_cn + "'" + " data-dishid='" + item.dish_id + "' />" 
						+ "<div class='info'" + " data-dishid='" + item.dish_id + "' >" 
						+ "<div class='visible_area'>"
						+ "<span class='left'>" + item.dish_name_cn + "</span>"
			...
			$("<h4>对不起，没有找到您需要的菜品. </h4>").insertAfter('#dishbox');
		}
	...
	function createDetailPage(res){
		...
		if(res.dishinfo && res.dishinfo.dish_id){
			...
			if(res.dishimages && res.dishimages.length>0){
				$('#pic_large').append(
						"<img  id='org_pic' data-path='" + res.dishimages[0].album_path + "' src='./image_800/" + res.dishimages[0].album_path + "'" + " alt='" + res.dishinfo.dish_name_cn + "'  width='460' height='350' style='opacity:1;'>"	
				);
			...
			$('#dish_text').append(
				"<h1 style='text-align:center'>" + res.dishinfo.dish_name_cn + "</h1>"
				+ "<ul>"
					+ "<li>"
					...
					+ "<li>"
						+ "<div class='dt'>分类:</div>"
						+ "<div class='dd'>" + res.cateinfo.cate_name_cn + "</div>"
					+ "</li>"
					+ "<li>"
						+ "<div class='dt'>辣:</div>"
					...
					+ "</li>"	
					+ "<li>"
						+ "<div class='dt'>现在价格:</div>"
						+ "<div class='dd' style='color:red' >" + "$" + res.dishinfo.current_price + "</div>"
					+ "</li>"	
					+ "<li>"
						+ "<div class='dt'>正常价格:</div>"
						+ "<div class='dd' ><s>" + "$" + res.dishinfo.reg_price + "</s></div>"
					+ "</li>"					
				+ "</ul>"
			);	
			
			$('#dish_desc').append("<h1>" + res.dishinfo.dish_desc_cn + "</h1>");				
		}else{
			$('#popup_page').append("<h1>对不起,没有菜品的详细信息. </h1>");
		}			
	}
